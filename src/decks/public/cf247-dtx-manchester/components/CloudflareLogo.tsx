/**
 * Cloudflare wordmark/logo as an inline SVG so it always renders
 * (no dependency on /logos/cloudflare.svg loading in time for chrome).
 *
 * SVG path lifted from cf-workers-design.nireka-96.workers.dev.
 */
type Props = {
  className?: string;
...
export function CloudflareLogo({ className, style, title = "Cloudflare" }: Props) {
  return (
    <svg
      className={className}
      style={style}
